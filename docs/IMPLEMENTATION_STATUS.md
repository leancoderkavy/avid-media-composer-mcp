...
| Saved project evidence | Paginated semantic diffs/source usage, bounded track/range pages, subclips, timecode, stereo channel mappings and per-track structural counts; per-bin source-usage warnings; generated transition overlap remains opaque and unresolved mixed-rate offsets stop downstream mapping | Live/unsaved graph access, deeper nested/effect/retime mapping, qualified rate conversion, effect/render-cost analysis and broader turnover reports |
...
The installed Windows Claude CLI passed isolated connection, removal and broken-startup recovery against a fresh development-package installation. Its 129-tool definitions matched that checkout exactly. The package check now compares full tool definitions on every run; the current inventory is 130 after adding saved-sequence structure reporting. This CLI evidence does not establish GUI or model-driven Avid invocation. Zod is pinned to the qualified version after fresh-install dependency drift changed advertised tuple constraints.
...
The latest `npm run check` passed with 362 TypeScript tests, 32 Python tests, 130 tools, stdio/HTTP checks, dry packing and fresh-tarball installation including actual Python MCP isolation and recovery. Log: `.avid-mcp-analysis/check-turnover-coverage.log`. Fresh installation also matched all checkout tool definitions exactly and invoked installed diff/source-usage/timeline pagination. The TypeScript run used the four-worker default. Real Sonoma forced-exit recovery and damaged-record pagination have separate retained MCP evidence. Tool count includes existing offline/bridge tools; it does not represent 130 native Avid operations. CI, local package checks, real Windows/Avid evidence, model/media quality and release proof remain distinct.
