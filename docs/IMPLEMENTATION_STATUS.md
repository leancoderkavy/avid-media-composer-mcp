...
The latest `npm run check` passed with 354 TypeScript tests, 29 Python tests, 129 tools, stdio/HTTP checks, dry packing and fresh-tarball installation including actual Python MCP isolation and recovery. Log: `.avid-mcp-analysis/check-job-history-recovery.log`. A subsequent focused regression test passed for continuation past an inaccessible history page; the five job-journal tests pass. Real Sonoma forced-exit recovery and damaged-record pagination have separate retained MCP evidence. Tool count includes existing offline/bridge tools; it does not represent 129 native Avid operations. CI, local package checks, real Windows/Avid evidence, model/media quality and release proof remain distinct.
