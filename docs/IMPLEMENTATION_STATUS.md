...
| Saved project evidence | Semantic snapshots/diffs, source usage, bounded track/range queries, subclips, timecode, observed stereo channel combiners and per-track structural counts; generated transition overlap mapping stays explicitly opaque | Live/unsaved graph access, deeper nested/effect/retime mapping, effect/render-cost analysis and broader turnover reports |
...
The latest `npm run check` passed with 357 TypeScript tests, 30 Python tests, 130 tools, stdio/HTTP checks, dry packing and fresh-tarball installation including actual Python MCP isolation and recovery. Log: `.avid-mcp-analysis/check-sequence-structure.log`. Fresh installation also matched all checkout tool definitions exactly. Real Sonoma forced-exit recovery and damaged-record pagination have separate retained MCP evidence. Tool count includes existing offline/bridge tools; it does not represent 130 native Avid operations. CI, local package checks, real Windows/Avid evidence, model/media quality and release proof remain distinct.
