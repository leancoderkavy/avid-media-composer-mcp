...
The latest `npm run check` passed with 338 TypeScript tests, 26 Python tests, 126 tools, stdio/HTTP checks, dry packing and fresh-tarball installation. Log: `.avid-mcp-analysis/check-aaf-merge-backend.log`. Tool count includes existing offline/bridge tools; it does not represent 126 native Avid operations. CI, local package checks, real Windows/Avid evidence, model/media quality and release proof remain distinct.
