...
`start` and `end` retain the requested extraction range; `analyzedStart` and `analyzedEnd` report the actual decoded PCM range. Alignment, scope counts and partial-range assignment checks use the decoded range. If extraction ends early, the remaining interval is unanalyzed and must not be interpreted as analyzed audio without speech. Assigning a speaker to a segment extending beyond decoded coverage requires explicit `allowPartialRange`, even when it fits inside the originally requested range.
