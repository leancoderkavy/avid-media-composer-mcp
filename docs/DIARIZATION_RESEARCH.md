...
A full installed Python distribution inventory found that the original venv inherited pip 23.2.1, with seven distinct CVEs listed by PyPI. New setup avoids the old installer entirely by creating a venv without pip and bootstrapping a checksum-pinned 26.2.1 wheel. Fresh installation passed; exact-release PyPI queries listed no active advisories for its four Python distributions. Legacy runtimes are retained and separately flagged as having an old bootstrap. Native-library advisories and the core wheel's missing license/notice file remain open; this is not a complete runtime security or redistribution acceptance. See [dependency audit](DIARIZATION_DEPENDENCY_AUDIT.md) for exact hashes, responses and evidence.
