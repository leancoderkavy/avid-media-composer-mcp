...
## Diagnostic range precision

Gap stops now identify only the uncovered interval; overlap stops identify the intersection. Component/track/bounds stops carry the affected clipped node range rather than the whole requested range. Mixed-rate stops additionally report originRate, targetRate, sourceBin, sourceMobId and sourceRangeConverted:false. Traversal still stops at that boundary: this does not implement or qualify mixed-rate conversion. The preceding reference coordinates retain the existing origin-rate offset calculation and must not be interpreted as converted target frames.

Build and 13 source-trace tests passed. The actual saved Sonoma MCP harness passed again with three source-resolution pages and 24 reference/unresolved trace steps: `.avid-mcp-analysis/source-resolution-mcp-26271672-13c5-4271-808d-e63204570dbb`. Mixed-rate/gap precision is tested with synthetic structures; the Sonoma fixture remains equal-rate.

Reference steps now carry originRate, targetRate (null for unresolved or ambiguous targets), and sourceRangeBasis. Equal-rate-offsets identifies the existing arithmetic where a unique target has the same rate. Unconverted-offsets labels mixed-rate or unknown-target calculations; those numbers are not qualified target-frame ranges. Equal rates alone do not prove target bounds, track identity, terminal media or playback. Build and 13 tests passed, and the real Sonoma MCP harness retained its 24-step result (`source-resolution-mcp-5574b140-8579-4968-a70e-0704b2d33318`).
