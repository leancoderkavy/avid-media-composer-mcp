...
Inspection accepts only the recognized operation ID and constant effect parameters, with two distinct, synchronized, direct sound references to one master. Unknown effects, extra inputs, channel timing differences and unsupported track formats are rejected. This is not general effect support. The builder reopens its output, checks every source mapping and runs the composition inspector before reporting conformance.
