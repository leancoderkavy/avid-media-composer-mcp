...
## Selects preservation checks, 2026-09-06

The production builder now verifies stored properties of every preserved source mob and its reachable weak definitions, then ties conformance to a hash rechecked after inspection and at the TypeScript boundary. `sourceGraphVerified: true` reports that bounded comparison; implicit schemas and stream-backed graphs are not covered. A new composition is the only mob excluded from source comparison and remains subject to the existing track/cut conformance checks. The retained preview/slideshow reference pair passed through MCP merge/build/inspection with these checks. See the completion ledger for evidence and negative tests. This strengthens authoring verification without expanding the same-rate straight-cut/stereo subset or claiming new host fidelity.
