...
## Native marker identity after bin reopen

The owned marker-bearing fixture was subsequently closed and reopened through verified native preview/apply operations. All three native `guid` fields then changed from the original UUIDs to the exact non-UUID IDs recorded in the restored AVB. Other native marker fields remained equal. The saved bin SHA-256 stayed `5051ae2cddfc102ff191b4257306cc6da731421ff9886277cc93a0711d1ebaa4`; decoded MOBs/warnings and original source hashes stayed unchanged.

Evidence: `.avid-mcp-analysis/marker-trim-reopen-b542f396-6647-48c2-ac7b-76f4f661251a/evidence.json`, generated by `qualify-marker-trim-reopen.mjs`. This disproves UUID continuity across this reload. Native `guid` is not necessarily a UUID after editor changes. Refresh native records after reload and never replay an old identifier. The current batch-delete schema accepts UUIDs only, so these reloaded markers are outside its qualified input contract. No marker write was attempted in this experiment; accepting or editing non-UUID identities requires separate native qualification. Application restart remains untested for this case.
