...
Current model-invocation preflight (2026-09-06): the explicit 2.1.260 executable above reports loggedIn:false via `auth status --json`. A signed-in client is required for the outstanding model-selected-tool test. This is separate from the previously successful unauthenticated CLI MCP connection checks. The older executable at `C:/Users/kavyr/.local/bin/claude.exe` reports 2.1.37; use the intended absolute binary path when reproducing qualification.
