...
The fresh-package smoke check runs the installed setup CLI for all five formats from a separate working directory, verifies their server command/environment agree (with VS Code's required `stdio` type), and launches that generated command for real MCP discovery and handler checks. This catches package-relative entry and configuration regressions; it does not launch the five named applications.
...
A published runtime with a setup lock can still have `unchanged: true`, but its inference preflight now reports `setup_lock_present` and `passed: false`. New runtime imports refuse that state. The explicit installer can verify its own exact lock record internally and only returns successfully after releasing it. This does not stop models already loaded in other processes or make setup and inference mutually exclusive; concurrent-operation ownership and automatic recovery remain open.
...
After a mixed-rate component, sequence offsets are unresolved: the inspector omits later nodes in that sequence and propagates this uncertainty to containing sequences. `UNRESOLVED_SEQUENCE_OFFSETS` explains the omission. A known same-rate prefix is retained. This supersedes the earlier behavior that retained later same-rate nodes despite an unresolved preceding duration; no rate conversion is inferred.

Source-usage `coverage` lists each authorized snapshot bin, its completeness and stored warning count. At most ten warning details per bin are returned; `warningsTruncated` indicates additional stored warnings. These include mixed-rate and unresolved-offset omissions even when no requested source references are returned. Zero matches with incomplete coverage are not proof that media is unused.

Snapshot diffs return `coverage.baseline` and `coverage.candidate` with per-bin completeness and up to ten stored warning details per bin. Inspect both sides even when `changes` is empty: zero visible differences do not establish equivalence if either capture omitted structure. Coverage differences are evidence limitations, not automatically editorial changes.

`avid_saved_snapshots` discovers historical revision IDs from the configured output library. Pages scan up to 50 validly named files, with a 10,000-entry directory ceiling; invalid files and inaccessible snapshots are counted as unavailable. Follow `nextAfter` even on empty pages. Summaries contain revision, capture time, bin/mob counts and completeness. They do not prove current bin hashes match the capture. Direct snapshot queries remain strict.
...
`avid_saved_snapshot_mobs` lists identities and structural metadata for a recovered revision, with `after`/`limit` (1–100), `totalMobs` and `nextAfter`. Entries retain their bin path, captured bin hash, mob ID/name/type, duration/rate and track count. Repeated IDs across bins remain separate entries; single-mob queries still require an unambiguous capture. This is historical metadata, not current host state.
...
Use `avid_saved_source_resolution` with `revision`, optional `after` (default -1), and `limit` (1–200) to page every direct source ID across a snapshot. Follow `nextAfter` with the same revision until null. Each row includes reference count, resolution status, candidate count and up to ten candidate records. `candidatesTruncated` signals when mob discovery is needed for all candidates. This tool does not classify terminal IDs or verify media availability.

`avid_saved_timeline_range` and `avid_saved_sequence_complexity` accept an optional absolute `bin` path returned by snapshot mob discovery. Use it when a MOB ID occurs in more than one captured bin. Keep the same bin, revision, MOB and range when paging. The query uses the authorized historical record and does not require the original bin file to still exist; omitting `bin` continues to reject ambiguous IDs.

The managed-removal research harness also qualifies a server launched directly from the renamed removal directory. It requires recovery refusal while that MCP server is running, then verifies recovery and a fresh MCP ping after shutdown. Both disposable installations are removed only after these checks. Windows evidence for the 135-tool branch: `.avid-mcp-analysis/package lifecycle 2ccd2c4a-358e-4fa3-8854-f80074ad1ecc/removal-evidence.json`. Reproduce after building with `qualify-package-install.mjs`, then pass its retained evidence file to `qualify-package-removal.mjs`. This uses two copies of the same package; it does not establish compatibility between versions.

Explicit CLIP, Whisper and Florence model setup now retains the recorded original-project notice under `MODEL_DIR/notices/OWNER/MODEL/REVISION/UPSTREAM.LICENSE`. The bundled source notice must match its recorded checksum. Existing matching notices are reused; changed notices and redirected notice directories are refused without overwrite. This happens before runtime/model setup and does not imply that weight installation completed. Ordinary offline model loading does not write notices or require migration of an older cache. The notice is provenance material, not a declaration that the complete conversion license chain has been cleared; see `OPTIONAL_MODEL_PROVENANCE.md`. Summary and diarization notice delivery remain separate work.

To qualify notice delivery through actual model setup, build and run `node scripts/research/qualify-model-notice-setup.mjs ABSOLUTE_MODEL_CACHE`. This explicitly runs the CLI setup for CLIP, Whisper tiny.en, Whisper tiny and Florence; missing dependencies or weights may be downloaded. It verifies each returned model/revision, checks the retained notice, and checkpoints results under `.avid-mcp-analysis/model-notice-setup-*`. All four passed against the existing Windows cache in `model-notice-setup-3f651605-cb3e-4177-8718-ad44764072af/evidence.json`. Cache-hit and network transfer amounts were not measured, and this is not clean-machine or inference-accuracy qualification.

Notice publication stages complete bytes before linking them exclusively to the final path, so concurrent callers cannot see an in-progress write as the final notice. The cache filesystem must support hard links. A failed or interrupted setup may leave a `.notice-UUID.creating` file; its presence alone does not prove that the writer stopped, and setup does not automatically remove other writers' staging files. This publication behavior has been qualified on the current Windows filesystem, not under power loss.

Interruption qualification: `node scripts/research/qualify-model-notice-interruption.mjs` injects a partial staging write in an owned child, confirms that the final notice is absent, terminates that child and awaits its exit, then retries actual notice setup. The retry must publish the correct bytes and preserve the old staging file; a subsequent call must reuse the completed notice. This passed on the current Windows filesystem in `.avid-mcp-analysis/notice-interruption-05d8cb00-27b8-4fd3-a6a7-56be0800b458/evidence.json`. It is controlled filesystem-fault/process-exit evidence, not power-loss durability or automatic stale-file cleanup.

Whisper base is an optional multilingual choice at pinned revision `1846881b6b3a3024392c1eea3ad983695bc23925`. Explicitly install it with `node dist/cli.js --download-models --model-dir ABSOLUTE_CACHE --speech --speech-model base`, then pass `options: {model: "base", language: "en"}` (or `"auto"`) to transcription. Automatic selection uses base's own first-window language scores; the separate language-detection tool still uses tiny. Default transcription remains tiny.en. Existing fixture comparisons did not show a text-quality benefit for base and measured slower inference; choose it deliberately rather than assuming larger means better. Its upstream notice follows the existing Whisper provenance limitations. Long-run interruption/resume and broader accuracy qualification remain separate work.

Claude model-invocation preflight (2026-09-06): the explicit 2.1.260 executable above reports loggedIn:false via `auth status --json`. Claude model-selected-tool testing remains pending sign-in. This is separate from the previously successful unauthenticated CLI MCP connection checks. The older executable at `C:/Users/kavyr/.local/bin/claude.exe` reports 2.1.37; use the intended absolute binary path when reproducing qualification.

Codex CLI 0.153.4 subsequently passed an actual authenticated model-selected read workflow. Run `node scripts/research/qualify-codex-model.mjs ABSOLUTE_CODEX_EXECUTABLE` with an already signed-in client. The harness uses `exec --ignore-user-config --ephemeral --sandbox read-only`, per-invocation MCP configuration, an inspect-only synthetic fixture root, and two enabled tools. It verifies model calls to health and hashed inventory, exact returned filename/hash, unchanged fixture bytes, and absence of other completed tool types. It does not write client configuration or copy credentials. Evidence: `.avid-mcp-analysis/codex-model-decfa8d3-bb9e-4b20-86cc-c47ff328f576/evidence.json`. This uses the checkout server and existing authenticated Windows client; it does not qualify fresh-machine setup, GUI behavior, or model-driven native editing.

For the fresh distribution path, run `node scripts/research/qualify-installed-codex-model.mjs ABSOLUTE_CODEX_EXECUTABLE`. It packs and installs the current build in a new managed directory, binds the model harness to the installed entry and SHA-256, and checks archive/client/server and fixture preservation. The actual model-driven health/inventory workflow passed: `.avid-mcp-analysis/installed-codex-model-0215ccd0-c9a1-41bb-b621-dfa3eb2a967f/evidence.json`. This qualifies a fresh package on an existing Windows machine, not fresh-machine dependencies or native Avid edits.
