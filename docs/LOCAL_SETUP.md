...
## Installed HTTP transport acceptance

`npm run smoke:package -- --with-python` now exercises the freshly installed
package's HTTP entry from a foreign working directory as well as its generated
stdio configuration. The HTTP check uses an ephemeral port, a random token and
inspection-only authority over a disposable ALE fixture. It checks health,
refuses missing and incorrect credentials, compares every HTTP tool definition
with installed stdio discovery, and verifies the same two-row ALE result without
changing source bytes. Provider and telemetry settings are not inherited.

The September 11, 2026 registry check independently installed published `1.1.0`
and passed this transport comparison with 27 tools. Initial connection took
1.12 seconds for stdio and 0.74 seconds for HTTP on the existing Windows host;
these are one-run observations, not performance guarantees. A deliberately
changed expected tool definition was rejected by the HTTP comparison.
The published package exposes the server entry only; development setup commands
and features are not implied by that result. Named-client GUI, registry-package
Python analysis, official MCP conformance and clean-OS qualification were not run
in this registry check. No Avid session was modified.

## Codex with an installed package and managed Python

Actual Codex model-selected inspection passed using configuration generated by a freshly installed package with an explicitly selected managed Python environment. A temporary read-only invocation exposed only `avid_analyze_bin`. The model called it exactly once for the Sonoma selects bin; its result matched an independent Python inspector read and reported five bin items/composition mobs at the requested bounded depth.

The managed environment tree, source bin, package entrypoints and existing Codex `config.toml` remained unchanged. Evidence: `.avid-mcp-analysis/codex-managed-python-1d982365-49d7-472e-b5f0-2cbed636628f/evidence.json`. Reproduce with `node scripts/research/qualify-codex-managed-python.mjs ABSOLUTE_CODEX_EXECUTABLE ABSOLUTE_INSTALLED_CLI_JS ABSOLUTE_MANAGED_PYTHON_DIRECTORY`. This uses generated settings as ephemeral overrides; it does not install persistent user configuration or qualify GUI onboarding, native editing or a clean OS.
...
On Windows, failed commands now request taskkill tree termination before direct-child fallback and wait for the tree command and child closure. Error details include `treeTermination.succeeded`; false means descendant termination was not confirmed, even if the direct child closed. Non-Windows retains direct-child termination. Actual ordinary/detached descendant tests passed for Windows timeout and overflow; see PROCESS_TREE_QUALIFICATION.md for normal-exit, parent-exit-race and cleanup limits. `treeTermination.outcome` adds classified taskkill counts (terminated, not found, access denied, unclassified, root not found) without retaining message text.
