...
it("binds managed installation contents and refuses changed files or receipt checksums",async()=>{
 const root=await mkdtemp(path.join(os.tmpdir(),"avid-managed-tree-")),id=randomUUID(),directory=path.join(root,id);await mkdir(directory);await writeFile(path.join(directory,"owned.txt"),"original");
...
it("rejects traversal and receipts bound to another location",async()=>{
 const root=await mkdtemp(path.join(os.tmpdir(),"avid-managed-location-")),id=randomUUID(),directory=path.join(root,id);await mkdir(directory);
...
it("refuses recovery when quarantined installation content is incomplete",async()=>{
 ...
 const root=await mkdtemp(path.join(os.tmpdir(),"avid-removal-recovery-")),id=randomUUID(),name=id+".removing-"+randomUUID(),directory=path.join(root,name);await mkdir(directory);
