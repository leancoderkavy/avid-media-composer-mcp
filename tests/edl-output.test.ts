import {mkdtemp,writeFile,symlink} from 'node:fs/promises';import os from 'node:os';import path from 'node:path';import {describe,it,expect} from 'vitest';
...
describe('native EDL output validation',()=>{
 ...
 it('rejects prior output through a canonical parent alias',async()=>{
 const root=await mkdtemp(path.join(os.tmpdir(),'edl-alias-')),alias=root+'-alias';
 await symlink(root,alias,process.platform==='win32'?'junction':'dir');
 const output=path.join(root,'sequence.edl');await writeFile(output,contents);
 await expect(verifyNativeEdlOutput(root,[{path:output}],[path.join(alias,'sequence.edl')],contract)).rejects.toThrow('existed');
 await expect(verifyNativeEdlOutput(root,[{path:output}],[path.join(root,'disappeared.edl')],contract)).rejects.toThrow();
 });
