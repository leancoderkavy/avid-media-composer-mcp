...
async function fixture(){
 ...
 await writeFile(path.join(directory,`${id}.json`),JSON.stringify({id,file:source,metadata:{format:{duration:4},streams:[{codec_type:'video'}]},transcript:[]}));return {id,config,captions:new FrameCaptions(config)};
}
it.each([false,true])('drains whole caption batches before closing their model (first failure=%s)',async fail=>{
 const {id,config,captions}=await fixture(),batches=new CaptionBatches(config,captions);let enter!:()=>void,release!:()=>void;
 ...
 const first=batches.generate(id,[0,1]).then(result=>({result}),error=>({error}));await entered;
 const queued=batches.generate(id,[2,3]),closing=batches.dispose();expect(batches.dispose()).toBe(closing);
 await expect(batches.generate(id,[0])).rejects.toThrow('closing');await expect(batches.resume('new')).rejects.toThrow('closing');expect(model.dispose).not.toHaveBeenCalled();
 release();const firstResult=await first;expect('error' in firstResult).toBe(fail);const completed=await queued;await closing;
 expect(await batches.status(completed.runId)).toMatchObject({state:'completed',completedCaptions:2});expect(model.generate).toHaveBeenCalledTimes(fail?3:4);expect(model.dispose).toHaveBeenCalledOnce();
 await expect(captions.generate(id,0)).rejects.toThrow('closing');
 if('error' in firstResult)expect(await batches.status(firstResult.error.details.runId)).toMatchObject({state:'partial',completedCaptions:0});
});
