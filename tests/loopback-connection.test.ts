...
it.skipIf(process.platform!=="win32").each(["127.0.0.1","::1"] as const)("verifies the accepted %s tuple before releasing bytes and refuses stale identity",async loopback=>{
 ...
 server.listen(0,loopback);await once(server,"listening");const address=server.address();if(!address||typeof address==="string")throw new Error("Missing socket address");
 let socket:Socket|undefined;
 try{
  const input={port:address.port,address:loopback,binary:process.execPath,sha256:await sha256File(process.execPath)};
  const owner=await verifyWindowsLoopbackOwner(input);
  socket=createConnection({host:loopback,port:address.port});await once(socket,"connect");const peerPort=socket.localPort!;
