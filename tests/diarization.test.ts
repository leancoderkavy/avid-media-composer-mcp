...
it("retains quarantined files when process verification fails after the move",async()=>{
 ...
 expect(await f.speakers.recoverCleanup(quarantine,checkpoint.sha256)).toMatchObject({recovered:true,analysisId,sourceModified:false});expect(await f.speakers.checkpoint(analysisId)).toEqual(checkpoint);await expect(readdir(path.join(f.base,quarantine))).rejects.toThrow();
});

it("refuses changed, partial, unauthorized and colliding cleanup recovery",async()=>{
 const f=await fixture(),normal=mocks.run.getMockImplementation()!;mocks.run.mockImplementationOnce(normal).mockResolvedValueOnce({exitCode:1});await expect(f.speakers.generate(f.id,0,2)).rejects.toThrow();const analysisId=(await f.speakers.list(f.id)).discovery.unpublishedAnalysisIds[0]!,checkpoint=await f.speakers.checkpoint(analysisId);
 mocks.stopped.mockResolvedValueOnce(undefined).mockRejectedValueOnce(new Error("retained"));await expect(f.speakers.cleanup(analysisId,checkpoint.sha256)).rejects.toThrow();const {readdir,rmdir}=await import("node:fs/promises"),name=(await readdir(f.base)).find(name=>name.startsWith("speaker-cleanup-"))!,directory=path.join(f.base,name),audio=path.join(directory,"speech.f32"),bytes=await readFile(audio);
 await expect(f.speakers.recoverCleanup("../outside",checkpoint.sha256)).rejects.toThrow("Invalid");await expect(f.speakers.recoverCleanup(name,"0".repeat(64))).rejects.toThrow("checkpoint mismatch");await expect(new SpeakerAnalysis({...f.config,capabilities:new Set(["inspect"])}).recoverCleanup(name,checkpoint.sha256)).rejects.toThrow();
 await writeFile(audio,"changed");await expect(f.speakers.recoverCleanup(name,checkpoint.sha256)).rejects.toThrow();await unlink(audio);await expect(f.speakers.recoverCleanup(name,checkpoint.sha256)).rejects.toThrow("partial");await writeFile(audio,bytes);
 const destination=path.join(f.base,`speakers-${analysisId}`);await mkdir(destination);await expect(f.speakers.recoverCleanup(name,checkpoint.sha256)).rejects.toThrow("already exists");expect(await readFile(audio)).toEqual(bytes);await rmdir(destination);
 mocks.stopped.mockRejectedValueOnce(new Error("active writer"));await expect(f.speakers.recoverCleanup(name,checkpoint.sha256)).rejects.toThrow("active writer");expect(await readFile(audio)).toEqual(bytes);expect(await sha256File(f.source)).toBe(f.id);
 mocks.stopped.mockResolvedValueOnce(undefined).mockImplementationOnce(async()=>{await writeFile(f.source,"changed during recovery");});await expect(f.speakers.recoverCleanup(name,checkpoint.sha256)).rejects.toThrow("files were restored");expect(await readFile(path.join(destination,"speech.f32"))).toEqual(bytes);
});
