...
export async function pythonRuntimeStatus(directory:string){
 ...
 if((await lstat(file)).isSymbolicLink())throw new Error("Python runtime receipt cannot be a link");
 ...
 return {directory,executable,receipt,treeSha256,unchanged:treeSha256===receipt.treeSha256,bootstrapCurrent:receipt.versions.pip===PIP_VERSION,
...
export async function installPythonRuntime(directory:string,basePython:string){
 ...
 try{
  ...
  return await pythonRuntimeStatus(directory);
