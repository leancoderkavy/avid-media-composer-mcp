...
type OwnerInput={port:number;address:"127.0.0.1"|"::1";binary:string;sha256:string;expectedIdentity?:string};
export async function verifyWindowsLoopbackOwner(input:OwnerInput){return verifyOwner(input);}

/** Server-side accepted TCP tuple, checked before the caller releases HTTP bytes. */
export async function verifyWindowsLoopbackConnection(input:OwnerInput&{peerPort:number}){
  if(!Number.isInteger(input.peerPort)||input.peerPort<1||input.peerPort>65535)throw fail("Invalid connection peer port");
  return {...await verifyOwner(input,input.peerPort),peerPort:input.peerPort};
}
async function verifyOwner(input:OwnerInput,peerPort?:number){
  ...
  const selector=peerPort===undefined?"-State Listen":`-State Established -RemotePort ${peerPort}`;
  const peerFilter=peerPort===undefined?"":` -and $_.RemoteAddress -eq '${input.address}'`;
  const result=await runProcess("powershell.exe",["-NoProfile","-NonInteractive","-Command",
    `$ErrorActionPreference='Stop'; @(Get-NetTCPConnection ${selector} -LocalPort ${input.port} | Where-Object { $_.LocalAddress -eq '${input.address}'${peerFilter} } | ForEach-Object { $p=Get-Process -Id $_.OwningProcess; @{path=$p.Path;pid=$p.Id;started=$p.StartTime.ToUniversalTime().ToString('o')} }) | ConvertTo-Json -Compress`],{timeoutMs:30000,maxOutputBytes:8192});
