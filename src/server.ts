...
export function createServer(config: ServerConfig = loadConfig()): McpServer {
  ...
  server.registerTool("avid_native_read", {
    description: "Opt-in Windows native app/project/bin/clip/marker/track inspection. Requires AVID_MCP_NATIVE_BINARY and allowed project roots. Tracks requires bin and mobId, and returns labels, flags and segment counts with schema defaults; it does not return clip source ranges or a complete live timeline graph.",
