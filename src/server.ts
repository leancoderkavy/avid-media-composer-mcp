...
export function createServer(config: ServerConfig = loadConfig()): McpServer {
  ...
  server.registerTool("avid_native_read", {
    description: "Opt-in Windows native app/project/bin/clip/marker/track inspection. Requires AVID_MCP_NATIVE_BINARY and allowed project roots. Media_volumes returns host-wide volume display names and raw free_space strings with unknown units/freshness, not canonical paths or online/relink status; it requires an authorized current project. Edl_settings lists preset names without verifying preset content or exporting files. Open_bins returns bounded canonical paths in the current authorized project with project checks before and after enumeration; it is not atomic. Viewer_bins discovers bounded, authorized Avid-reported bins for current viewers without a bin argument; it brackets identity and membership but does not verify keyboard focus or atomic state. Viewers requires bin and omits entries outside it. Tracks requires bin and mobId, and returns labels, flags and segment counts with schema defaults; it does not return clip source ranges or a complete live timeline graph.",
    inputSchema: { query: z.enum(["app", "project", "media_volumes", "bins", "mob_bin", "viewer_bins", "open_bins", "bin", "bin_columns", "clips", "selected_clips", "clip", "clip_columns", "markers", "tracks", "viewers", "link_settings", "export_settings", "edl_settings", "import_settings"]), bin: z.string().optional(), mobId: z.string().optional() },
