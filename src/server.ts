...
export function createServer(config: ServerConfig = loadConfig()): McpServer {
  ...
  server.registerTool("avid_native_read", {
    description: "Opt-in Windows native app/project/bin/clip/marker/track inspection. Requires AVID_MCP_NATIVE_BINARY and allowed project roots. Open_bins returns bounded canonical paths in the current authorized project with project checks before and after enumeration; it is not atomic. Viewers requires bin and omits entries outside it. Tracks requires bin and mobId, and returns labels, flags and segment counts with schema defaults; it does not return clip source ranges or a complete live timeline graph.",
    inputSchema: { query: z.enum(["app", "project", "bins", "open_bins", "bin", "clips", "clip", "markers", "tracks", "viewers", "link_settings", "export_settings", "import_settings"]), bin: z.string().optional(), mobId: z.string().optional() },
