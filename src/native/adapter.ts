...
export class NativeAdapter {
  ...
  private async state(action: Action) {
    ...
    if(action.action==="copy_clip"||action.action==="copy_clips"){
      ...
      const requested=action.action==="copy_clip"?[action.mobId]:action.mobIds;
      if(requested.some(id=>!clips.some(clip=>clip.mob_id===id)))throw new Error("Copy target is not in source bin");
    ...
    const task = queue.catch(() => {}).then(async () => {
      ...
      return withNativeLock(async () => {
        ...
        switch (action.action) {
          ...
          case "delete_marker": {
            if(project.path!==observedState.project||this.client.ownerIdentity!==observedState.owner)throw new Error("Marker host or project changed before dispatch");
            result = await this.client.call("DeleteMarkers", { mob_id: action.mobId, guid: [action.guid] },observedState.owner); break;
          }
          case "delete_markers": {
            if(project.path!==observedState.project||this.client.ownerIdentity!==observedState.owner)throw new Error("Marker host or project changed before dispatch");
            if(!("markers" in observedState))throw new Error("Missing marker baseline");
        ...
        try {
          if(action.action==="add_marker"){
            ...
            if(!("markers" in observedState))throw new Error("Missing marker baseline");
            const remaining=(observedState.markers as Record<string,any>[]).filter(marker=>action.action==="delete_marker"?marker.guid!==action.guid:typeof marker.guid!=="string"||!action.guids.includes(marker.guid.toLowerCase()));
          ...
          if(action.action==="copy_clip"||action.action==="copy_clips"){
            const requested=action.action==="copy_clip"?[action.mobId]:action.mobIds;
            const reported=z.array(z.object({mob_id:z.array(id).max(4096)})).length(1).parse(result)[0]!.mob_id;
            ...
            if(reported.length!==requested.length||new Set(reported).size!==reported.length||digest(identities(copied))!==digest([...reported].sort())||!("clips" in observedState)||digest(identities(source))!==digest(identities(observedState.clips)))throw new Error("Copy identity or source membership not verified; inspect both bins before another attempt");
