...
export async function verifyNativeRender(file:string,config:ServerConfig,input:Contract,options:{timeoutMs?:number;pollMs?:number;assertOwner?:()=>Promise<void>}={}){
  ...
  const deadline=Date.now()+timeoutMs;let previous:string|undefined,lastReason="Output not observed",identity:string|undefined;
  ...
  while(Date.now()<deadline){
    ...
    if(resolved){
      ...
      if(info.size>0&&previous===stamp){
        const probe=await runProcess(config.ffprobeExecutable,["-v","error","-protocol_whitelist","file,pipe","-show_streams","-of","json",resolved],{timeoutMs:remaining(),maxOutputBytes:1024*1024});
        ...
        if(probe.exitCode===0&&matchesRenderContract(metadata,expected)){
          ...
          if(decode.exitCode===0&&decode.stdout.includes("progress=end")&&decodedFrames===expected.frames&&Date.now()<=deadline&&finalPath===resolved&&`${after.size}:${after.mtimeMs}:${after.ino}`===stamp&&await sha256File(resolved)===sha256){
            return {output:resolved,bytes:after.size,sha256,expected,metadata,decodedFrames,decodePassed:true,contractMatched:true,sourceFidelity:"Not assessed; this verifies the declared output contract",exportRetried:false};
          }
  ...
  throw new Error(`Render readiness unproven: ${lastReason}. Inspect this output and host before any new export; no export was retried.`);
}
