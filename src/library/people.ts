...
const recordSchema=z.object({schema:z.literal(1),revision:z.string().uuid(),modelRevision:z.literal(FACE_REVISION),faces:z.array(faceSchema).max(1000),clusters:z.array(clusterSchema).max(1000),threshold:z.number().min(0).max(1)});
...
export class People {
  ...
  private async read(indexId:string){
    ...
    await this.library.metadata([...new Set(record.faces.map(face=>face.mediaId))]);
  ...
  async index(ids:string[],samples:number,threshold=0.45){
    ...
    if(!ids.length||ids.length>20||!Number.isInteger(samples)||samples<1||samples>24)throw new Error("People indexing is limited to 20 media files and 24 samples each");
    ...
    const frames=[];
    for(const entry of await this.library.metadata([...new Set(ids)])){
      ...
      const duration=Number(entry.metadata.format?.duration);if(!Number.isFinite(duration)||duration<=0)throw new Error("Invalid media duration");
      for(let sample=0;sample<samples;sample++){
        const time=duration*(sample+0.5)/samples,file=path.join(directory,`frame-${frames.length}.jpg`);
    ...
    const record:RecordData={schema:1,revision:randomUUID(),modelRevision:FACE_REVISION,faces,clusters,threshold};
    await writeFile(path.join(directory,"index.json"),JSON.stringify(record),{flag:"wx"});
    return {indexId,revision:record.revision,faces:faces.length,clusters:clusters.length,samples:frames.length,reviewRequired:true,meaning:"Visual similarity groups; names are supplied by the user, not inferred"};
  }
  async list(indexId:string,after=-1,limit=50){
    ...
    return {indexId,revision:record.revision,clusters:groups.slice(0,limit),nextAfter:groups.length>limit?groups[limit-1]?.index:null,faces:record.faces.length,reviewRequired:true};
  }
  ...
  async remove(indexId:string,expectedRevision:string){
    ...
    try{
      ...
      if(files.length>2000)throw new Error("Index directory exceeds removal limit");
