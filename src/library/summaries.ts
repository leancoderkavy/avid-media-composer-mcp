...
export class MediaSummaries{
  ...
  async node(revision:string,nodeId?:string){
    ...
    const recipeChunks=record.chunkRecipe===undefined?null:summaryChunks(source.segments,record.chunkRecipe);
    const leafIndex=record.nodes.filter(candidate=>!candidate.children.length).findIndex(candidate=>candidate.nodeId===node.nodeId);
    const inputText=recipeChunks===null?null:node.children.length?node.children.map(id=>byId.get(id)!.summary).join(" "):recipeChunks[leafIndex]!.text;
    const modelInput=inputText===null?{status:"not_recorded" as const,text:null,sha256:null,kind:null,childNodeIds:[] as string[]}:{status:"reconstructed_recipe" as const,text:inputText,sha256:createHash("sha256").update(inputText).digest("hex"),kind:node.children.length?"generated_children" as const:"transcript_chunk" as const,childNodeIds:node.children};
    const sourceExcerpts=recipeChunks===null?null:recipeChunks.flatMap((chunk,i)=>leafIds.has(`n${i}`)?chunk.spans.map(span=>({...span,leafNodeId:`n${i}`,text:segments.get(span.index)!.text.slice(span.charStart,span.charEnd)})):[]);
    return {revision,sha256,id:record.id,transcriptRevision:record.transcriptRevision,model:record.model,node,children:node.children.map(id=>byId.get(id)!),modelInput,modelInputMeaning:"Reconstructed from the recorded recipe and current verified source/child text, not an independent generation log. Overview input contains generated child summaries; compare against original sources to review inherited errors and omissions.",sources:source.segments.filter(s=>indices.has(s.index)),sourceExcerpts,sourceExcerptsStatus:sourceExcerpts===null?"not_recorded":"verified_recipe",sourceExcerptsMeaning:"Exact original UTF-16 character spans used by descendant leaves, in chunk order. Segment timestamps are not character-level timings; quotes do not verify generated claims.",quality:{potentiallyTruncatedNodeIds,subtreeMayBeTruncated:potentiallyTruncatedNodeIds.length>0,scope:"Selected node and all descendants",note:"Sentence-ending heuristic only; an empty list does not verify completeness or factual accuracy."},sourceScope:node.children.length?"descendant_leaves":"direct_leaf",reviewRequired:true,factualEntailmentVerified:false};
  }
