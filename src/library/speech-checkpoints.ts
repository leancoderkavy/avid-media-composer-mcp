...
export class SpeechCheckpoints{
  ...
  async create(input:Pick<Header,"id"|"start"|"end"|"options"|"audioHash"|"plannedWindows"|"parentRunId"|"languageDecision"> & {recipe?:1|2|3}){
    const runId=randomUUID(),selected=speechModels[input.options.model],record=header.parse({...input,recipe:input.recipe??(input.languageDecision?2:1),runtime:"4.2.0",runId,model:selected.model,modelRevision:selected.revision,createdAt:new Date().toISOString()});
