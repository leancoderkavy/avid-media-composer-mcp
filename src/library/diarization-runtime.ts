import {PIP_VERSION,preparePipWheel} from "./python-bootstrap.js";
...
const receiptSchema=z.object({schema:z.literal(1),kind:z.literal("avid-diarization-runtime"),pipVersion:z.literal(PIP_VERSION).optional(),installationId:z.string().uuid(),treeSha256:hash,workerSha256:hash,versions:z.object({"sherpa-onnx":z.literal("1.13.7"),"sherpa-onnx-core":z.literal("1.13.7"),numpy:z.literal("2.2.6")}).strict(),checkedAt:z.string(),checks:z.object({binaryOnly:z.literal(true),dependencyCheckPassed:z.literal(true),silenceInferencePassed:z.literal(true)}).strict()}).strict();
...
export async function diarizationRuntimeStatus(cache:string){
  ...
  return {root,directory,executable,receipt,treeSha256,bootstrapCurrent:receipt.pipVersion===PIP_VERSION,unchanged:treeSha256===receipt.treeSha256&&workerSha256===receipt.workerSha256,note:"Local tree consistency and prior dependency/inference checks; not a current vulnerability audit, publisher authentication or accuracy acceptance."};
}
...
export async function installDiarizationRuntime(cache:string,python:string){
  ...
  try{
    if(await exists(path.join(root,"installation.json"))){const status=await diarizationRuntimeStatus(cache);if(!status.unchanged)throw new Error("Diarization runtime tree or worker changed; choose a fresh model directory");if(!status.bootstrapCurrent)throw new Error("Diarization runtime uses legacy pip setup; choose a fresh model directory for the verified bootstrap");return {...status,reused:true};}
    ...
    await execute(python,["-B","-m","venv","--copies","--without-pip",path.join(directory,"runtime")]);
    const executable=await resolveReadablePath(pythonAt(directory),[directory],"file");
    const wheel=await preparePipWheel(directory);
    await execute(executable,["-B","-c","import sys,runpy; sys.path.insert(0,sys.argv.pop(1)); runpy.run_module('pip',run_name='__main__')",wheel,"install","--no-index","--no-deps","--no-compile","--disable-pip-version-check",wheel]);
    if((await execute(executable,["-B","-c","import importlib.metadata; print(importlib.metadata.version('pip'))"])).stdout.trim()!==PIP_VERSION)throw new Error("Pip bootstrap version mismatch");
    ...
    const receipt=receiptSchema.parse({schema:1,kind:"avid-diarization-runtime",pipVersion:PIP_VERSION,installationId,treeSha256:before,workerSha256,versions:DIARIZATION_VERSIONS,checkedAt:new Date().toISOString(),checks:{binaryOnly:true,dependencyCheckPassed:true,silenceInferencePassed:true}});
