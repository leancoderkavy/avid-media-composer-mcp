...
export function registerLibraryTools(server: McpServer, config: ServerConfig) {
  ...
  server.registerTool("avid_transcribe_media", {description:"Transcribe up to ten minutes of local audio with explicitly downloaded English or multilingual Whisper weights. Select an explicit language code for non-English audio: auto falls back to English because language detection is unavailable. Requires export and project-write. Returns a reviewable transcript revision; no diarization or accuracy guarantee.",inputSchema:{id,start:z.number().nonnegative(),end:z.number().positive(),options:speechOptions.default({model:"tiny.en",language:"auto"})},annotations:write},
