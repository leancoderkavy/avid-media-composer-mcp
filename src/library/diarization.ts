...
export class SpeakerAnalysis{
  ...
  async align(analysisId:string,analysisSha256:string,transcriptRevision:string,transcriptSha256:string,after=-1,limit=100,candidateLimit=20){
    ...
    const {record}=saved,spans=effectiveSpans(record),analyzedRange={start:record.start,end:record.start+record.machine.duration};
    const intersecting=transcript.record.segments.map((segment,index)=>({segment,index})).filter(({segment})=>segment.start<analyzedRange.end&&segment.end>analyzedRange.start);
    const matching=intersecting.filter(({index})=>index>after);
    const segments=matching.slice(0,limit).map(({segment,index})=>({index,segment,...alignSpeakerSegment(segment,analyzedRange,spans,candidateLimit)}));
    ...
    return {analysisId,analysisSha256,transcriptRevision,transcriptSha256,id:record.id,start:record.start,end:record.end,analyzedStart:analyzedRange.start,analyzedEnd:analyzedRange.end,totalTranscriptSegments:transcript.record.segments.length,intersectingSegments:intersecting.length,outsideAnalysisSegments:transcript.record.segments.length-intersecting.length,segments,nextAfter:matching.length>limit?segments.at(-1)!.index:null,scope:"Transcript segments intersecting the analyzed source range; indices refer to the original transcript revision. Counts cover the entire revision, independent of this page; intersecting segments may extend beyond the analyzed range.",assignmentApplied:false,reviewRequired:true,wordAlignmentVerified:false,speakerIdentityVerified:false,note:"Candidates reflect interval overlap, not confidence or verified speaker attribution. Multiple sequential or simultaneous voices remain ambiguous. Original transcript text and speaker fields are unchanged. Candidate lists may be explicitly truncated; inspect saved speaker pages for all intervals."};
  }
  assign(analysisId:string,analysisSha256:string,transcriptRevision:string,transcriptSha256:string,input:z.input<typeof speakerAssignments>){return this.serialize(async()=>{
    ...
    const record=saved.record,spans=effectiveSpans(record),analyzedRange={start:record.start,end:record.start+record.machine.duration},names=new Map<string,string>();
