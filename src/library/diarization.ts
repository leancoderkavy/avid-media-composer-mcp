...
const audioCheckpoint=z.object({schema:z.literal(1),analysisId:uuid,id:sha,start:z.number().nonnegative(),end:z.number().positive(),audioRecipe:z.literal(3),audioSha256:sha,audioBytes:z.number().int().positive().max(38400000),workerSha256:sha,runtimeTreeSha256:sha,options:diarizationOptions}).strict().refine(value=>value.end>value.start&&value.end-value.start<=600&&value.audioBytes%4===0&&value.audioBytes<=Math.ceil((value.end-value.start)*16000)*4,"Invalid speaker checkpoint range");
...
export class SpeakerAnalysis{
  ...
  async checkpoint(analysisId:string){
    ...
    const audio=await resolveReadablePath(path.join(directory,"speech.f32"),[directory],"file"),pcm=await readBoundedFile(audio,checkpoint.audioBytes);if(pcm.length!==checkpoint.audioBytes||createHash("sha256").update(pcm).digest("hex")!==checkpoint.audioSha256)throw new Error("Speaker checkpoint audio changed");
  ...
  private async generateRun(id:string,start:number,end:number,input:z.input<typeof diarizationOptions>,parent?:Awaited<ReturnType<SpeakerAnalysis["checkpoint"]>>){
    ...
    const checkpoint=audioCheckpoint.parse({schema:1,analysisId,id,start,end,audioRecipe:3,audioSha256,audioBytes:pcm.length,workerSha256:runtime.receipt.workerSha256,runtimeTreeSha256:runtime.treeSha256,options});
  ...
  remove(analysisId:string,expectedSha256:string){return this.serialize(async()=>{
    requireCapability(this.config.capabilities,"project-write");sha.parse(expectedSha256);const current=await this.record(analysisId);if(current.sha256!==expectedSha256)throw new Error("Speaker analysis changed");const files=await readdir(current.directory);if(!files.includes("analysis.json")||!files.includes("speech.f32")||files.some(file=>!["analysis.json","speech.f32","audio.json"].includes(file)))throw new Error("Unexpected speaker files; removal refused");
    if(await sha256File(current.file)!==expectedSha256)throw new Error("Speaker analysis changed before deletion");if(files.includes("audio.json")){const checkpoint=await this.checkpoint(analysisId);if(checkpoint.audioSha256!==current.record.machine.audioSha256)throw new Error("Speaker checkpoint mismatch");await unlink(path.join(current.directory,"audio.json"));}await unlink(current.file);await unlink(current.audio);await rmdir(current.directory);return {analysisId,removed:true,sourceModified:false};
  });}
}
