...
export async function loadCaptionModel(cache:string,download=false){if(download)await installModelNotice(cache,CAPTION_MODEL,CAPTION_REVISION);const runtime=await modelRuntime(cache,download),options={cache_dir:cache,revision:CAPTION_REVISION,local_files_only:!download};const location=download?CAPTION_MODEL:path.resolve(cache,CAPTION_MODEL,CAPTION_REVISION);const processor=await runtime.AutoProcessor.from_pretrained(location,options) as import("@huggingface/transformers").Florence2Processor,model=await runtime.Florence2ForConditionalGeneration.from_pretrained(location,{...options,dtype:"q4"});return {processor,model,RawImage:runtime.RawImage,Tensor:runtime.Tensor};}
