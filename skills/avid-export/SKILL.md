...
Stereo metadata alone does not prove channel preservation. The Sonoma AAC-linked fixture exported duplicate channels; a separately prepared source-clock PCM/stereo-master path preserved exact 24-bit PCM. On that prepared sequence, refreshing Color Adapters on an isolated copy before export improved the decoded color comparison while retaining exact audio. This is bounded fixture evidence, not an automatic ingest or color remedy. Do not silently relink a user's sequence, alter panning or retag an export based on it.

For color investigation, capture saved bins and inspect `avid_saved_timeline_range` effect declarations/fingerprints. A recorded LUT label, level range or automatic-conversion flag describes saved metadata; it does not prove that the rendered color math is correct. `avid_trace_saved_sources` marks recognized color inputs as diagnostic and incomplete. Native Color Adapters refresh is not a shipped MCP operation; when reproducing the qualified workflow, use a separately owned sequence copy and an explicitly observed Avid UI action, then verify save/reopen and the exported media. Compare decoded audio and source-clock video timing in addition to container tags.
