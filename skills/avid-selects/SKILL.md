...
2. Build reviewed integer source-frame cuts with `avid_build_aaf_selects` at the exact master-slot rate and explicit picture/sound slot mappings. Inspect the result using `avid_inspect_aaf_selects`. Preserve the new AAF checksum, track mapping and source-media evidence.
