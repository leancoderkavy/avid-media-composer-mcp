...
for(let index=2;index<process.argv.length;index+=2){const key=process.argv[index];assert.ok(['--server-entry','--python'].includes(key)&&process.argv[index+1]&&!options.has(key));options.set(key,process.argv[index+1]);}
const serverEntry=path.resolve(options.get('--server-entry')??'dist/index.js'),serverEntrySha256=await sha256File(serverEntry);
const pythonExecutable=path.resolve(options.get('--python')??'.venv/Scripts/python.exe');
...
try{
 ...
 await writeFile(path.join(root,'evidence.json'),JSON.stringify({serverEntry,serverEntrySha256,pythonExecutable,merged,built,inspected,hostImportVerified:false},null,2),{flag:'wx'});console.log(path.join(root,'evidence.json'));
