...
const original=fs.writeFile;fs.writeFile=async function(file,...args){const result=await original(file,...args);if(path.resolve(String(file))===path.join(process.env.AVID_RESEARCH_DIRECTORY,'attempt.json')){await original(process.env.AVID_RESEARCH_READY,JSON.stringify({pid:process.pid}));await new Promise(()=>setInterval(()=>{},1000));}return result;};syncBuiltinESMExports();`);
const child=spawn(process.execPath,['--import',pathToFileURL(loader).href,cli,'--install-python-runtime',directory,'--python',basePython],{stdio:['ignore','ignore','pipe'],windowsHide:true,env:{...process.env,AVID_RESEARCH_DIRECTORY:directory,AVID_RESEARCH_READY:ready}});
...
try{
 const deadline=Date.now()+15000;let observed;
 ...
 await writeFile(path.join(root,'evidence.json'),JSON.stringify({passed:true,before,after,termination,attemptHash,retry,scope:'Production CLI paused immediately after attempt publication and before its first subprocess. Status reported incomplete while live and after confirmed owned-child closure; retry refused and attempt retained. Not mid-pip termination, power loss or descendant containment.'},null,2),{flag:'wx'});
