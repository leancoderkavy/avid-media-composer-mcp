...
const input=process.argv[2];assert.ok(input&&path.isAbsolute(input)&&process.argv.length===3,'Pass absolute completed marker trim evidence directory');
const root=path.resolve('.avid-mcp-analysis',`installed-marker-trim-${randomUUID()}`);await mkdir(root);
...
const result=await run(['scripts/research/qualify-marker-trim-refusal.mjs',input,installation.entry]);
...
await writeFile(path.join(root,'evidence.json'),JSON.stringify({installation,archiveSha256,result,scope:'Fresh managed package, actual retained trim AVBs, forward/inverse identity-change refusal and raw saved marker reads after MCP reconnect. Existing Windows/Python host; no UI execution or exact undo acceptance.'},null,2),{flag:'wx'});
