...
try{
  ...
  const started=await call('avid_start_analysis_job',{job:{kind:'speech',id,start:0,end:180}});
  ...
  const baseline=await call('avid_transcribe_media',{id,start:0,end:180});assert.deepEqual(completed.result.segments,baseline.segments,'Resumed segments must equal uninterrupted result');
