...
function compare(start,end,sourceChannel,renderChannel,lag=0,stride=1){
  ...
  return {correlation:denom>0?(ab-sumA*sumB/count)/denom:null,gain:aa>0?ab/aa:null,rmse:Math.sqrt(error/count),sourceRms:Math.sqrt(aa/count),renderRms:Math.sqrt(bb/count),samples:count};
}
const cuts=[];
