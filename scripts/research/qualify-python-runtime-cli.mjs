...
const [directory]=process.argv.slice(2);assert.equal(process.argv.length,3);assert.ok(path.isAbsolute(directory));
...
const cli=path.resolve('dist/cli.js'),events=[];
...
for(let n=0;n<2;n++){
 ...
 try{
  await client.connect(new StdioClientTransport({command:process.execPath,args:[path.resolve('dist/index.js')],cwd:root,stderr:'pipe',env:{...getDefaultEnvironment(),PYTHONPATH:root,AVID_MCP_PYTHON:before.executable,AVID_MCP_CAPABILITIES:'inspect',AVID_MCP_ALLOWED_ROOTS:files.map(file=>path.dirname(file)).join(path.delimiter)}}));
...
await writeFile(path.join(root,'evidence.json'),JSON.stringify({passed:true,before,after,files,hashes,sourceUnchanged:true,reconnectUnchanged:true,scope:'Production CLI installation status and overwrite refusal, doctor, real AVB/AAF MCP reads and unchanged runtime tree across reconnect. Existing Windows/base Python; no removal, upgrade, clean OS or wider Python workload qualification.'},null,2),{flag:'wx'});
