...
const recoveryName=prior.first.installationId+".removing-"+randomUUID();assert.equal(path.dirname(prior.first.directory),root);await rename(prior.first.directory,path.join(root,recoveryName));const recovery=await cli(["--package-recover",recoveryName,"--package-root",root,"--expected-sha256",status.receiptSha256]);assert.equal(recovery.recovered,true);
