...
import {clientConfiguration} from '../../dist/setup.js';

const binary=process.argv[2];
if(process.platform!=='win32')throw Error('This qualification harness currently covers Windows only');
if(!binary||!path.isAbsolute(binary)||process.argv.length!==3)throw Error('Provide the absolute installed Claude CLI executable');
...
const entry=clientConfiguration('generic',[project]).mcpServers['avid-media-composer'];
const results=[];
...
try{
  ...
  await writeFile(path.join(root,'evidence.json'),JSON.stringify({ok:true,version,binary,binarySha256:await hash(binary),serverEntry:entry.args[0],serverEntrySha256:await hash(entry.args[0]),configuration,project,protectedConfigurationsUnchanged:true,results,limitations:['CLI connection and configuration lifecycle only','No model inference, GUI tool invocation or native Avid action','Existing Node and Claude installation used']},null,2));
