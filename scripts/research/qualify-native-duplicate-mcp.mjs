...
assert.ok(process.argv.length===2||process.argv.length===3&&process.argv[2]==='--history');
const history=process.argv[2]==='--history',expectedCount=history?3:2;
const mobId='060a2b340101010501010f1013-000000-b92a4d5912898806-82a0d8bbc16d-18d9';
assert.equal(await sha256File(path.join(project,bin)),history?'91c4b94d67229540476fcda5c45b1ba767fa45275482d28e2a4bc26f95c735a0':'8395e0435ad36a5197f431df7262e297e7c4815f942329cdd9d57993e4a95df1');
...
try{
  ...
  assert.equal(before.length,expectedCount);assert.ok(before.some(row=>row.mob_id===mobId));
  ...
  assert.equal(after.length,expectedCount+1);assert.ok(before.every(row=>after.some(item=>item.mob_id===row.mob_id&&item.mob_name===row.mob_name)));
  assert.deepEqual(await Promise.all(protectedFiles.map(sha256File)),hashes);
  await writeFile(path.join(root,'evidence.json'),JSON.stringify({passed:true,historyProbe:history,bin,mobId,before,applied,after,protectedFiles,hashes,sourceUnchanged:true,
