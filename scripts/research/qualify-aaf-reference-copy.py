...
assert all(arg in ['--prepared-reference','--remap-collisions'] for arg in sys.argv[1:])
...
try:
 ...
 report.update(verified=True,sourceHashesUnchanged=True,limitations=['Two distinct masters refer to the same prepared media','Not independent-media or native import qualification','Experimental MobID remapping; complete descriptor/weak-reference equivalence remains unverified'])
