...
try{
  ...
  if(expectStopped){
    ...
    after=await client.callTool({name:'avid_native_lock_status',arguments:{}});
    assert.ok(!after.isError,JSON.stringify(after));assert.equal(after.structuredContent.data.locked,false);
    const archive=JSON.parse(await readFile(result.structuredContent.data.archive,'utf8'));
    assert.equal(archive.lock.sha256,data.sha256);assert.equal(archive.exportRetried,false);assert.equal(archive.outputDeleted,false);
    await assert.rejects(readFile(lock),{code:'ENOENT'});
  }else{
    assert.equal(result.isError,true);assert.match(JSON.stringify(result),/Close Avid Media Composer/);assert.equal(await readFile(lock,'utf8'),lockText);
  }
  assert.equal(await readFile(output,'utf8'),'test artifact');
  await writeFile(path.join(root,'evidence.json'),JSON.stringify({status,result,after,expectStopped,outputPreserved:true,scope:'Isolated retained-lock fixture against actual installed Avid process state'},null,2));
  console.log(JSON.stringify({passed:true,expectStopped,outputPreserved:true,evidence:path.join(root,'evidence.json')}));
