...
const snapshots=[];
const snapshot=async label=>{const source=path.join(project,bin),file=path.join(root,label+'.avb'),before=await sha256File(source);await copyFile(source,file,1);assert.equal(await sha256File(file),before);assert.equal(await sha256File(source),before);snapshots.push({label,file,sha256:before});};
try{
 ...
 if(!resume)await snapshot('before-markers');
 ...
 verify(persisted);
 await snapshot(resume?'resume-persisted-markers':'persisted-markers');
 let restored=null;
 ...
 if(!retainMarkers)await snapshot(resume?'resume-cleaned-markers':'cleaned-markers');
 await writeFile(path.join(root,resume?'resume-snapshots.json':'snapshots.json'),JSON.stringify(snapshots,null,2),{flag:'wx'});
