...
const resume=process.argv[2];assert.ok(!resume||(path.isAbsolute(resume)&&process.argv.length===3),'Optional absolute failed batch evidence root');
...
try{
 ...
 const markers=recorded?.action.markers??[{guid:randomUUID(),offset:15,track:{type:'TRACKTYPE_PICTURE',number:1},name:'QC first',comment:'Batch first reviewed marker',color:'Green'},{guid:randomUUID(),offset:75,track:{type:'TRACKTYPE_PICTURE',number:1},name:'QC second',comment:'Batch second reviewed marker',color:'Yellow'}];
 const verify=items=>{assert.equal(items.length,original.length+markers.length);for(const marker of markers){const actual=items.find(item=>item.guid===marker.guid);assert.ok(actual);for(const key of ['offset','name','comment','color'])assert.equal(actual[key],marker[key]);assert.equal(actual.length,1);assert.equal(actual.user,'Avid MCP');assert.deepEqual({type:actual.track_label.type??'TRACKTYPE_PICTURE',number:actual.track_label.number},marker.track);}for(const marker of original)assert.ok(items.some(item=>JSON.stringify(item)===JSON.stringify(marker)));};
 ...
 verify(persisted);
 for(const marker of markers)await apply({action:'delete_marker',bin,mobId,guid:marker.guid});await reopen();
 const restored=await call('avid_native_read',{query:'markers',bin,mobId});assert.deepEqual(restored,original);
 assert.deepEqual(await Promise.all([sourceFile,media].map(sha256File)),before);
 await writeFile(path.join(root,'evidence.json'),JSON.stringify({bin,mobId,markers,applied,persisted,restored,sourceHashes:before,sourceUnchanged:true,markerBaselineRestored:true,scope:'Two markers on one owned 30 fps copied sequence, saved/reopened then explicitly deleted and marker list restored. No atomicity, whole-bin byte restoration, cross-rate or bulk-scale claim.'},null,2),{flag:'wx'});
 console.log(JSON.stringify({root,bin,markerBaselineRestored:true,sourceUnchanged:true}));
