...
const fixture=JSON.parse(await readFile(path.join(JSON.parse(await readFile(path.join(previous.input,'evidence.json'),'utf8')).input,'fixture.json'),'utf8')),baseline=JSON.parse(await readFile(path.join(input,'cleaned.json'),'utf8'));
...
const root=path.resolve('.avid-mcp-analysis',`native-single-creation-${randomUUID()}`);await mkdir(root);
const client=new Client({name:'native-single-creation',version:'1.0'}),events=[];
...
try{
 ...
 const notes=[];
 for(const [index,offset] of [0,75].entries()){
  const note={offset,track:{type:'TRACKTYPE_PICTURE',number:1},name:index?'Preserve note':'Created at zero',comment:'Single creation qualification',color:'Green'};
  const created=await apply({action:'add_marker',bin:fixture.bin,mobId:fixture.mobId,...note});assert.equal(created.markerAddedVerified,true,JSON.stringify(created));
  notes.push({...note,guid:created.result[0].guid});
 }
 await reopen();const before=await read();assert.equal(before.length,2);const original=await capture('original');
 ...
 await writeFile(path.join(root,'evidence.json'),JSON.stringify({input,notes,savedSurvivorVerified:true,cleanedBaselineGraph:true,sourceFilesUnchanged:true,scope:'Two native single-marker creations including frame zero, followed by save/reopen, outside-note preservation and explicit decoded baseline cleanup. One owned Windows fixture; no atomic undo or arbitrary ID qualification.'},null,2),{flag:'wx'});console.log(JSON.stringify({root,passed:true}));
