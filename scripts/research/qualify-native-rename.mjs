...
assert.ok(process.argv.slice(2).every(value=>['--persist','--unicode'].includes(value))&&new Set(process.argv.slice(2)).size===process.argv.length-2,'Only optional --persist and --unicode are supported');const persist=process.argv.includes('--persist');
const project='D:/Avid Projects/MCP_Sonoma_30p_20260905',bin='MCP_AAF_Selects_20260905.avb',mobId='060a2b340101010501010f1013-000000-3737af0e12888806-0e10d8bbc16d-18d9',original='MCP_Sonoma_AAF_Selects',temporary=process.argv.includes('--unicode')?'Café — 葡萄园 (prise 2)':'MCP Rename Qualification',source='D:/Sonoma Escape Edit/Sonoma_Escape_RoughCut_v1_preview.mp4';
...
try{
 if(process.argv.includes('--unicode')){
  assert.equal(persist,false,'Unicode refusal does not save or reopen bins');
  const before=await call('avid_native_read',{query:'clip',bin,mobId});
  const refused=await client.callTool({name:'avid_native_preview',arguments:{operation:{action:'rename_clip',bin,mobId,expectedName:original,name:temporary}}});assert.ok(refused.isError,'Unicode preview must be rejected');
  const after=await call('avid_native_read',{query:'clip',bin,mobId});
  assert.equal(after.find(row=>row.column_name==='Name')?.column_value,before.find(row=>row.column_name==='Name')?.column_value);
  assert.equal(await sha256File(source),sourceHash);assert.equal(await sha256File(path.join(project,bin)),binHashBefore);
  await writeFile(path.join(root,'evidence.json'),JSON.stringify({refused,before,after,sourceHash,binHashBefore,scope:'Unicode preview refused through MCP; no apply request, source/bin hashes and observed Name unchanged. Does not qualify Unicode writes.'},null,2));console.log(JSON.stringify({passed:true,refusal:true,root}));
 }else{
