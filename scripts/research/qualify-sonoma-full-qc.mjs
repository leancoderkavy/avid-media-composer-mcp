...
const probed=await runProcess('ffprobe',['-v','error','-select_streams','v:0','-count_frames','-show_frames','-show_entries','frame=pts:stream=nb_read_frames,time_base:format=start_time,duration','-of','json',file],{timeoutMs:120000,maxOutputBytes:1048576});assert.equal(probed.exitCode,0,probed.stderr);
...
try{
 ...
 const [num,den]=probe.streams[0].time_base.split('/').map(Number),pts=probe.frames.map(f=>f.pts),deltas=pts.slice(1).map((p,i)=>p-pts[i]);
 const expectedTiming={frames:pts.length,timeBase:{num,den},firstPts:pts[0],lastPts:pts.at(-1),duplicateSteps:deltas.filter(d=>d===0).length,backwardSteps:deltas.filter(d=>d<0).length,minDelta:Math.min(...deltas),maxDelta:Math.max(...deltas)};
 assert.deepEqual(report.videoTiming,expectedTiming);
 ...
 assert.deepEqual(saved.report.videoTiming,expectedTiming);
 assert.equal(await sha256File(file),id);
