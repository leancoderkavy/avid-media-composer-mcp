...
const token=randomUUID()+randomUUID(),server=createHttpServer({authToken:token,maxSessions:2,authenticatedRateLimitPerMinute:600,config:loadConfig({AVID_MCP_ALLOWED_ROOTS:path.dirname(source),AVID_MCP_OUTPUT_ROOT:root,AVID_MCP_MODEL_DIR:path.resolve('.avid-mcp-analysis/models'),AVID_MCP_CAPABILITIES:'inspect,export,project-write'})});
...
try{
 ...
 const sessionId=owner.transport.sessionId,deletion=owner.transport.terminateSession().then(()=>({ok:true}),error=>({error:String(error)}));
 const headers={Authorization:`Bearer ${token}`,'Content-Type':'application/json',Accept:'application/json, text/event-stream'};
 let stale;
 for(let attempt=0;attempt<20;attempt++){stale=await fetch(url,{method:'POST',headers:{...headers,'Mcp-Session-Id':sessionId},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'ping'})});await stale.text();if(stale.status===404)break;await new Promise(resolve=>setTimeout(resolve,25));}
 assert.equal(stale.status,404);
 const initialize=()=>fetch(url,{method:'POST',headers,body:JSON.stringify({jsonrpc:'2.0',id:1,method:'initialize',params:{protocolVersion:'2025-03-26',capabilities:{},clientInfo:{name:'capacity-probe',version:'1'}}})});
 const during=await initialize();await during.text();assert.equal(during.status,503,'Closing owner released capacity before its batch drained');
 assert.deepEqual(await deletion,{ok:true});await owner.client.close();
 const available=await initialize(),probeId=available.headers.get('mcp-session-id');await available.text();assert.equal(available.status,200);assert.ok(probeId);
 const removed=await fetch(url,{method:'DELETE',headers:{...headers,'Mcp-Session-Id':probeId}});await removed.text();assert.equal(removed.status,200);
 ...
 await writeFile(path.join(root,'evidence.json'),JSON.stringify({standalone,before,after,reply,staleStatus:stale.status,capacityDuring: during.status,capacityAfter:available.status,prefixUnchanged:true,sourceUnchanged:true,passed:true,scope:'Real HTTP session DELETE during a partially checkpointed direct Florence batch, with a standalone caption model also loaded. Closing owner holds the second session slot until cleanup resolves. Observer verifies completion and unchanged checkpoint/source. Not forced process exit, cancellable job semantics, accuracy or allocator reclamation.'},null,2),{flag:'wx'});
