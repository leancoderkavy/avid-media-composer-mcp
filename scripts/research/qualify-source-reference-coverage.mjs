...
const files=[path.join(project,'MCP_AAF_Selects_20260905.avb')];if(process.argv.includes('--with-copy'))files.push(path.join(project,'MCP_CopyMCP_a9c7f7a1ad75.avb'));
const snapshot=await service.create(files);await writeFile(path.join(root,'snapshot.json'),JSON.stringify(snapshot,null,2));
...
assert.equal(result.complete,true);assert.equal(result.sourceReferenceCoverage.allReferencesResolve,false);assert.equal(result.sourceReferenceCoverage.unresolvedCount,1);if(process.argv.includes('--with-copy'))assert.equal(result.snapshotSourceReferenceCoverage.ambiguousCount,4);console.log(JSON.stringify({root,coverage:result.sourceReferenceCoverage,snapshotCoverage:result.snapshotSourceReferenceCoverage}));
