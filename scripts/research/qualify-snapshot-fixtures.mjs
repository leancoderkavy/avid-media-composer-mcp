...
for name in ['subclip','stereo','opaque']:
 d=root/name; d.mkdir(); p=t.fixture(d) if name=='subclip' else t.stereo_fixture(d, (lambda e:setattr(e,'effect_id','OTHER_EFFECT')) if name=='opaque' else None); result.append(dict(name=name,file=str(p)))
...
try{
 for(const fixture of fixtures){
  ...
  assert.equal(report.duration,fixture.name==='subclip'?60:30);
  assert.equal(report.complete,fixture.name!=='opaque');
  assert.equal(report.opaqueNodes,fixture.name==='opaque'?1:0);
