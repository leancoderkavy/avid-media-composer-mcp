...
assert.ok(process.argv.slice(2).every(value=>['--stereo-legal','--pcm-selects','--imported-selects'].includes(value)), 'Only owned fixture variants are supported');
const importedSelects=process.argv.includes('--imported-selects');
const pcmSelects=process.argv.includes('--pcm-selects')||importedSelects;
...
try{
  ...
  if(importedSelects){operation.bin='MCP_Import_3405029e.avb';operation.mobId='060a2b340101010501010f1013-000000-aa4d1d5e12888806-f9bad8bbc16d-18d9';operation.expected.videoStartTime=0;operation.expected.audio[0].startTime=0;}
