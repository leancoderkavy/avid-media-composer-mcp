...
try{
  ...
  const output=path.resolve('.avid-mcp-analysis',`face-search-${randomUUID()}`);await mkdir(output);await writeFile(path.join(output,'evidence.json'),JSON.stringify({priorPath,result,bounded,filtered,sourceUnchanged:true,scope:'Read-only MCP feature ranking and filtering verified independently on saved Sonoma detections; not identity/recognition accuracy'},null,2));
