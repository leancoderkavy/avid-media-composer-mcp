...
assert.ok(process.argv.slice(2).every(arg=>arg==='--stereo-sequence'));
const stereoSequence=process.argv.includes('--stereo-sequence');
...
const mobId=stereoSequence?'060a2b340101010501010f1013-000000-aa4d1d5e12888806-f9bad8bbc16d-18d9':reference.mobId;
const binFile=path.join(projectPath,stereoSequence?'MCP_Import_3405029e.avb':reference.bin),binSha256=await sha256File(binFile);
// Re-inspected saved graph: same composition, cuts and stereo combiners;
// this current bin hash differs from its earlier import receipt.
if(stereoSequence)assert.equal(binSha256,'2d9cc121f7e2b374684749b3fa13aae65102c765a6dfa0258d9727edacdbe56b');
...
await withNativeLock(async()=>{
  ...
  const info=await call('GetMobInfo',{mob_id:mobId},owner);assert.ok(JSON.stringify(info).includes(stereoSequence?'MCP_PCM_AAF_Selects':'Sonoma_SourceClock_Stereo'));
  const settings=await call('GetListOfExportSettings',{},owner);assert.ok(settings.some(value=>value.setting_names?.includes('AAF')));
  await writeFile(path.join(root,'attempt.json'),JSON.stringify({mobId,output,preset:'AAF',owner,stereoSequence,binFile,binSha256}),{flag:'wx'});
  try{
    await call('ExportFile',{mob_id:mobId,file_name:'PCM_reference',export_settings_name:'AAF',destination_path:root,in_directory:'export',option_flags:['Export_StopIf_OfflineMedia','Export_StopIf_UnknownFX']},owner);
    ...
    assert.equal(await sha256File(reference.media),reference.mediaSha256);
    assert.equal(await sha256File(binFile),binSha256);
    const report={output,sha256:await sha256File(output),mobId,stereoSequence,binFile,binSha256,binUnchanged:true,sourceMedia:reference.media,sourceSha256:reference.mediaSha256,sourceUnchanged:true,exportRetried:false,limitations:['Stable AAF container only; inspect source descriptors and tracks before using the template.','This is a fixed research export, not a general MCP AAF export action.']};
