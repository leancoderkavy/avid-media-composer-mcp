...
Use it with an MCP client that can launch a local process or reach the configured authenticated HTTP server. The published `1.1.0` package provides analysis, interchange and bridge contracts, and it is what `npx` and the `latest` distribution tag install today. Version `1.2.0-rc.1` is cut in this repository and adds local media intelligence, installation helpers and a separate Windows native adapter, but it has not been published to npm yet; run it by building from source until that release candidate ships. A compatibility-rule result is not proof of this connector running on that host; native runtime evidence currently covers the qualified Windows Media Composer 2024.12 build described in [implementation status](docs/IMPLEMENTATION_STATUS.md). Mac implementation and host qualification are deferred.
...
> These changes are **not** in the public `1.1.0` package, and the `1.2.0-rc.1` candidate that contains them is not on npm yet. Skip to [Facts at a glance](#facts-at-a-glance) for the released server. See [local setup](docs/LOCAL_SETUP.md) for running this checkout and [implementation status](docs/IMPLEMENTATION_STATUS.md) for what each addition has and has not proven.

**Local media intelligence.** This checkout indexes local media by content hash and adds bounded search, QC and reporting over that index: technical metadata and facets, black/freeze/silence and loudness measurement, contact sheets, thumbnail strips, transcript and speaker workflows, visual and people indexes, and verified copies or trims that leave the source untouched. `avid_timecode_continuity` assesses declared container and stream timecode across indexed files, handling drop-frame numbering for 29.97 and 59.94, and reports contiguous, gap, overlap or incomparable pairs from metadata alone. Longer analysis runs as cancellable local jobs whose status survives reconnecting; see [audio sync research](docs/AUDIO_SYNC_RESEARCH.md) and [watch folder recovery](docs/WATCH_FOLDER_RECOVERY.md).

**Saved-project inspection.** Snapshots of saved AVB bins support paginated [marker inspection](docs/SAVED_MARKERS.md) with text, color declarations and explicitly unresolved or effect-input locations, timeline range and source-usage queries, sequence complexity, snapshot diffing, checksum verification against current bytes, filtered clip search described in [saved snapshot search](docs/SAVED_SNAPSHOT_SEARCH.md), and metadata-only file availability checks in [saved locator availability](docs/SAVED_LOCATOR_AVAILABILITY.md). These are saved-file facts and exclude unsaved editor state.

**Guarded native adapter.** On the qualified Windows build the adapter performs bounded operations with capability checks, single-use tokens and post-write verification: [clip Comments set and clear](docs/NATIVE_CLIP_COMMENTS.md), [AAF selects import](docs/NATIVE_AAF_IMPORT.md), [bin-column discovery](docs/NATIVE_BIN_COLUMNS.md), batch marker removal, clip duplication, clip renaming that checks the expected current name, media-volume declarations, and bin-scoped track and viewer inspection. [Source-viewer loading](docs/NATIVE_VIEWER_QUALIFICATION.md) verifies the requested MOB ID for the sequence case, while source-master identity mapping remains unresolved and Record and seek options are not exposed. None of this establishes full timeline editing or atomic undo.

**Runtime and recovery.** An isolated Python runtime can be installed into a new directory with a checksum-verified bootstrap and read-only status checks, so bin inspection no longer depends on ambient interpreter settings. Interrupted work is discoverable rather than replayed: watch folders recover crashed owner locks without discarding checkpoints, and [source-clock preparation](docs/SOURCE_CLOCK_PREPARATION.md) records attempts, publishes receipts as complete files and reports unresolved runs explicitly. Windows worker cancellation retains classified termination counts for diagnosis without storing raw command output.

**Workflow skills.** Original [workflow skills](docs/WORKFLOW_SKILLS.md) cover ingest/QC, selects, review markers, turnover and export.
