...
The following list describes the published analysis foundation. For the development branch's local visual search, transcription, people/speaker workflows, QC, saved snapshots and native operations, see the [current implementation and remaining acceptance work](docs/IMPLEMENTATION_STATUS.md). Tool discovery through `tools/list` is authoritative for the installed package; the development package currently exposes 139 tools, including offline and bridge tools.
